...
def test_get_progress_user_not_found(user_progress_table_instance: UserProgressTable) -> None:
    ...
    """
    result = user_progress_table_instance.get_user_progress("non-existent-user")
...
def test_get_progress_user_exists(user_progress_table_instance: UserProgressTable, dynamodb_table_object) -> None:
    ...
    test_data = {
        "userId": user_id,
        "completion": {"l1": {"s1": "time-5", "s2": "time-10"}},
    }
    ...
    # Test retrieval
    result = user_progress_table_instance.get_user_progress(user_id)
    assert result is not None
    assert result.userId == user_id
    assert "l1" in result.completion
    assert result.completion["l1"] == {"s1": "time-5", "s2": "time-10"}


@mock_aws
def test_get_unit_progress_for_user_1(user_progress_table_instance: UserProgressTable, dynamodb_table_object) -> None:
    """
    Test getting progress for existing user on specific unit they have done
    """
    user_id = "test-user-123"
    test_data = {
        "userId": user_id,
        "completion": {"u1/l1": {"s1": "time-5", "s2": "time-10"}},
    }
    ...
    # Test retrieval
    result = user_progress_table_instance.get_unit_progress_for_user(UserId(user_id), "u1")
    assert result is not None
    assert len(result) == 1
    assert result["u1/l1"] == {"s1": "time-5", "s2": "time-10"}


@mock_aws
def test_get_unit_progress_for_user_2(user_progress_table_instance: UserProgressTable, dynamodb_table_object) -> None:
    """
    Test getting progress for existing user on specific unit they haven't done yet
    """
    user_id = "test-user-123"
    test_data = {
        "userId": user_id,
    ...
    # Test retrieval
    result = user_progress_table_instance.get_unit_progress_for_user(UserId(user_id), "u2")
    assert result == {}
...
def test_update_progress_empty(user_progress_table_instance: UserProgressTable, dynamodb_table_object) -> None:
    ...
    user_progress_table_instance.update_user_progress(user_id, [])
    result = user_progress_table_instance.get_user_progress(user_id)
...
def test_update_progress_single_update(user_progress_table_instance: UserProgressTable, dynamodb_table_object) -> None:
    ...
    user_progress_table_instance.update_user_progress(
        user_id, [SectionCompletionInputModel(lessonId="l1", sectionId="s1")]
    )
    result = user_progress_table_instance.get_user_progress(user_id)
...
def test_update_progress_triple_update(user_progress_table_instance: UserProgressTable, dynamodb_table_object) -> None:
    ...
    user_progress_table_instance.update_user_progress(
        user_id,
        [SectionCompletionInputModel(lessonId="l1", sectionId="s3")],
    )
    result = user_progress_table_instance.get_user_progress(user_id)
